...
class SSL:
    ...
    def __Send_Read(self, message:str) -> str:
        ...
        try:
            ...
            context = ssl._create_unverified_context() #default context had a problem with certificate (no idea why... tried to fix it but failed) -> unverified_context: no server certificate check          
        ...
        try:
            ...
            conn.sendall(message_bytes) #send message
            ...
            try:
                ...
                answer = self.__read_from_conn(conn) #read answer
    ...
    def Send_Jsons(self, jsons:list) -> bool:
        """
        This method sends all measurements as json to server

        params:
        jsons: measurements as list containing jsons
        """
        ...
        jsons_appended = ";".join(stringified_jsons) #convert jsons and join them with ';' as separator
...
class BLE:
    ...
    def __Scan_For_Advertizements(self, timeout_s=10.0) -> dict:
        ...
        try:
            ...
            for advertisement in it: 
                ...
                addr = advertisement.address #get address of device

                #if address not found already
                if addr not in found_addr:
                    found_addr.add(addr) #add  to found
                    ...
                    if name in self.__device_names: #ceck if devicename matches with a name of predefined deviceName list
                        measurement_device_advertizements[name] = advertisement #store advertizement
        ...
        self.__Stop_Scan_Save() #stop scan
    ...
    def __Request_Measurements_from_connection(self, connection: BLEConnection, start_read_timeout_s=4.0):
        """
        This method requests all measurements from a device
        ...
        try:
            ...
            while True:
                ...
                byte_:bytes = uart.read(1) #read a byte
    ...
    def Start_Request(self, max_number_of_tries:int = 5):
        """
        Start measure request from all devices

        params:
        max_number_of_tries: number of reties if connection to a device failed
        """
        
        jsons:json = json.loads(json.dumps({deviceName:"BLE_error" for deviceName in self.__device_names})) #create json with bleError as standard entry for every device

        Write_To_Log_File("BLE", "start advertizement scan")
        adverts = self.__Scan_For_Advertizements() #scan for advertizements
        Write_To_Log_File("BLE", "successfully finished advertizement scan")

        timeStamp = datetime.now() #get current time (timestamp of measurement in database)
        for deviceName in list(adverts.keys()): #iterate over deviceNames
            ad = adverts[deviceName] #get advertizement

            number_of_tries = 0
            while number_of_tries < max_number_of_tries: #retry max X times -> if connection cannot be established bleError is the value of entry for this device
                print(f"Try connect to  {deviceName}")

                Write_To_Log_File("BLE", f"try connect to: {deviceName}")

                connection = self.__TryConnect_ToDevice(ad) #connect to advertizement

                if (connection != None): #if connection established
                    ...
                    answer = self.__Request_Measurements_from_connection(connection) #request and get measurements from device
                    ...
                    if answer != None:
                        jsons[deviceName] = answer #override bleError with received measurements

                        print(f"Successfully got data from {deviceName}")
                        Write_To_Log_File("BLE", f"Successfully got data from {deviceName}")

                    else: #getting measurements was not successful (bleError)
                ...
                else: #connection failed -> retry max X times
...
#init
server = SSL()
ble = BLE(["Device1", "Device2", "Device3"]) #all devices that master has to listen for is given as parameter here
cache = Cache()
...
while True:
    try:
        ...
        start_Time, jsons = ble.Start_Request() #get measurements (or errors) from all devices
        new_measurement = json.dumps({"timeStamp":start_Time.strftime("%d/%m/%Y %H:%M:%S"), "data":jsons}) #add timestamp and measurement results to a json
        Write_To_Log_File("Main", "successfully read data from ble and dumped to json")

        #read cached jsons
        Write_To_Log_File("Main", "start cache things")
        cached_jsons = cache.Cache_Read() #read cached mesurements
        ...
        if server.Send_Jsons(all_jsons): #try send all measuremens to server
        ...
        else: #sending measurements not successful
            Write_To_Log_File("Main", "send data to server failed -> cache data")
            print("Data Cached")

            #append new measurement to chache
